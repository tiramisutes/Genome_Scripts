...
sub convert {
	my $config = Bio::WGS2NCBI::Config->new;
	my $ORNAME  = $config->orname;
	my $INDIR   = $config->datadir;
	#my $INDIRF  = $config->tdatafile;
	my $TMPL    = $config->template;
	my $OUTDIR  = $config->outdir;
	my $DISCREP = $config->discrep;
	my $TBL2ASN = $config->tbl2asn;
	#my $var = (split("/",$INDIRF))[-1];
	my $dir = getcwd;
	print "The current run path is: $dir\n";
	chdir($OUTDIR) or die "$!";
	my $ndir = getcwd;
	print "The current command run path is: $ndir\n";
	my $fafiles = glob("*.fsa");
	print "The input genome fasta file is : $fafiles\n";
